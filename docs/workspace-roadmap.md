...
```
┌──────┬─────────────┬────────────────────────────────┐
│ 活動 │  Explorer   │  ┌─[file.ts][README][Spec:x]─┐  │
│ 列   │             │  │                            │  │
│      │  folder A   │  │   主內容區                 │  │
│ 📁   │   └ src/    │  │   （Monaco 編輯 / spek 頁）│  │
│ 📊   │  folder B   │  │                            │  │
│ 🔍   │   └ ...     │  └────────────────────────────┘  │
...
│      │             │  └────────────────────────────┘  │
└──────┴─────────────┴────────────────────────────────┘
  活動列      可調寬       可調高的底部 terminal dock
```

- 活動列：📁 Explorer / 📊 OpenSpec（Dashboard·Specs·Changes·Graph）/ 🔍 搜尋。
- 主區 tab 與底部 terminal dock 皆可多開、可關、寬高可調（split panes）。
- spek 視圖以 tab 開在主區，與一般檔案 tab 並存，達到「協調整合」。
